...
| QQ 默认 Skill 自动注入       | `design-docs/Agent/qq-chat-skill-auto-injection.md` | 待验收 | QQ 订阅边界已固定预启用 `qq-chat`，Harness 首轮直接进入 `skill_loaded` 并排除重复可请求目录；32 项定向测试通过，实现每文件覆盖率均超过 89%，真实模型 eval 已同步新协议。                                                                                                                                                                                               |
...
7. 扩展工具治理、RunTrace、risk/actions 对接、MCP 工具来源和长期记忆工具。
