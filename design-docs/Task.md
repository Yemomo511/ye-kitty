...
| 模块                         | 设计文档                                     | 状态   | 进度说明                                                                                                                                                                           |
| ---------------------------- | -------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Harness Agent 第二版整体方案 | `design-docs/Agent/agent-runtime-harness.md` | 待验收 | MVP 已实现 Harness 主循环、`get_recent_messages`、Prompt 分层、`skill_call` 结构化协议和 Skill 扩展字段；已强化 System Prompt 的环境感知与风险治理；完整校验剩余本地监听环境限制。 |
...
3. 接入市场 Skill 协议解析和 Ye-Kitty 运行时转换，保证 Skill 只声明能力和建议工具，不直接获得执行权。
