...
| 模块                         | 设计文档                                     | 状态   | 进度说明                                                                                   |
| ---------------------------- | -------------------------------------------- | ------ | ------------------------------------------------------------------------------------------ |
| Harness Agent 第二版整体方案 | `design-docs/Agent/agent-runtime-harness.md` | 设计中 | 已形成循环观察、Skill 协议、工具治理、权限、日志、迁移和验收方案，等待确认后进入开发拆分。 |
...
| 问题                            | 影响                                                                         | 下一步                                                                                                  |
| ------------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| risk/actions 正式链路尚未落地   | Harness 的高风险动作只能停留在候选动作设计，不能直接执行完整对外动作         | 第一阶段只允许 `reply`、`ignore`、`human_review` 和只读工具，后续在 risk/actions 落地后开放候选动作。   |
