...
- 第一版不实现控制面配置编辑，只定义运行时配置形态和日志观测口径。
...
- 入口：`packages/kitty-service/src/services/agent-runtime/application/model-router.ts`
...
- 入口：`packages/kitty-service/src/services/agent-runtime/application/model-worker.ts`
...
- 明确 @ 和私聊优先级高于普通群聊触发；低优先级请求过期后可以直接丢弃。
...
- 单元测试：覆盖模型路由选择、单节点并发限制、请求间隔、队列过期和 429 退避翻倍。
- 集成测试：用假 OpenAI 兼容客户端模拟成功、429、超时和不可重试错误。
- 回归测试：确认 `AgentRuntimeHarness` 的结构化决策协议不变，`AgentRunnerPort.decide()` 调用方不感知具体模型节点。
...
- 开发验收：存在可测试的模型池端口、路由、单节点 Worker 和 429 退避逻辑。
- Agent 验收：实现后同步 `Task.md`、测试结果和真实代码入口，不让文档停留在概念层。

## 唯一入口

- 使用入口：Agent Harness 请求结构化模型决策。
- 代码入口：`packages/kitty-service/src/services/agent-runtime/ports/agent-runner.port.ts`
- 测试入口：后续新增 `packages/kitty-service/src/services/agent-runtime/__test__/model-request-pool.test.ts`
