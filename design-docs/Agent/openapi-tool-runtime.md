...
数组、枚举和 `additionalProperties`；后续确需完整 Draft 兼容时再单独评审依赖。
...
- 入口：`packages/kitty-service/src/agent-runtime/tools/result.ts`
- 职责：统一校验、授权、超时、取消、幂等、Effect 和审计。
- 边界：不把完整系统结果暴露给模型。
...
- 开发验收：生产链无 JSON Action 解析、无旧 Schedule 执行旁路、全量检查通过。
...
| 日期       | 状态   | 说明                                                                                                      |
| ---------- | ------ | --------------------------------------------------------------------------------------------------------- |
| 2026-07-23 | 开发中 | 已确认 opencode 与官方 Agents SDK 边界，完成唯一 Tool、官方投影、系统结算、finish、审批和模型池改装设计。 |
