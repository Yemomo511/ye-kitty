...
- 重要细节：项目启动只调用 Docker Compose，不安装 Docker；Apple Silicon 自动选择上游 `latest-arm64`，其他架构使用默认镜像，用户显式配置始终优先；健康检查失败时不继续登录。
- 边界：默认只把 MCP 端口绑定到 `127.0.0.1`，不读取 Cookies 内容，不负责小红书账号风控。
...
| `YE_KITTY_XIAOHONGSHU_MCP_IMAGE` | 输入 | 可覆盖按宿主架构自动选择的上游镜像                   |
...
| 无鉴权 MCP 暴露到局域网  | 宿主机端口绑定所有网卡           | 默认只绑定 `127.0.0.1`，远程部署必须自行增加鉴权             |
...
| Cookie 文件权限过宽      | 上游按默认 `umask` 写出 `0644`   | 容器入口收紧已有文件到 `0600`，并设置 `umask 077`            |
...
- 产品验收：已通过。项目启动菜单存在小红书选项，未登录时自动打开二维码，扫码后明确确认成功，重启后直接识别已登录。
- 设计验收：登录进度、二维码路径、超时和失败原因均为中文，并提供下一步命令。
- 开发验收：60 项小红书 MCP 定向测试通过；新增核心文件每文件行覆盖率不低于 91.66%，分支覆盖率不低于 90.56%，函数覆盖率不低于 83.33%。Lint、Prettier、类型、架构检查和无效参数启动冒烟通过。全量测试只剩既有 OneBot `fetch_custom_face` 固定 1 秒响应测试超时，脱离本次修改仍可复现。
- Agent 验收：已通过。真实 MCP 发现并注册 13 个上游工具；登录与读取工具可执行，写工具仍被风险门禁拒绝。
...
| 日期       | 状态   | 说明                                                                                                                                                                                                              |
| ---------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-07-16 | 已完成 | 已完成启动选项、Docker 健康检查、13 个工具配置、二维码登录与状态轮询；不传镜像环境变量的真实启动自动选择 `latest-arm64`，扫码、容器重建和重启后登录状态检查均通过；MCP 只监听 `127.0.0.1`，Cookie 权限为 `0600`。 |
