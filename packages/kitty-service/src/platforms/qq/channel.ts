...
import { MessageIngressRuntime } from './ingress';
import { OneBotMessageIngressService } from './onebot/ingress';
import {
  isOneBotV11SupportedMessageEvent,
  type OneBotV11SupportedMessageEvent,
} from './onebot/schema';
import type { OneBotFastifyReverseWsServer } from './onebot/server';
 ...
 */
export class QqAccountExperimentChannel extends PlatformMessageService<PlatformMessage> {
  // OneBot事件转换器
  private readonly oneBotIngress = new OneBotMessageIngressService();
  // QQ统一事件转换器
  private readonly qqIngress = new MessageIngressRuntime();
