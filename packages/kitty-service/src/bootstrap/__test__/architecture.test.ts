...
describe('三层架构规则', () => {
  test('接受 Platform、Agent Runtime、Shared 和 Bootstrap 目标目录', async () => {
    const root = await createPackageFixture({
      ...
      'src/agent-runtime/__test__/agent.test.ts': "import '../agent';\n",
  ...
  test('拒绝散落在模块目录中的测试文件', async () => {
    const root = await createPackageFixture({
      'src/index.ts': '',
      'src/ARCHITECTURE.md': '# 架构\n',
      'src/agent-runtime/agent.ts': 'export const agent = true;\n',
      'src/agent-runtime/agent.test.ts': "import './agent';\n",
      'src/agent-runtime/__test__/nested/agent.test.ts': "import '../../agent';\n",
      'src/platforms/index.ts': '',
      'src/shared/index.ts': '',
      'src/bootstrap/start.ts': '',
    });
    ...
    expect(violations).toContain('测试文件必须位于__test__目录: src/agent-runtime/agent.test.ts');
    expect(violations).toContain(
      '测试文件必须位于__test__目录: src/agent-runtime/__test__/nested/agent.test.ts',
    );
  });
