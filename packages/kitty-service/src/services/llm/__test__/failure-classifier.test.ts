...
describe('classifyFailure', () => {
  ...
  it('T4-16: session timeout → session_timeout / retryable=false', () => {
    const f = classifyFailure({
      exitCode: null, killed: true, cancelRequested: false,
      stderrTail: '', phase: 'running', timedOutKind: 'session',
    });
  ...
  it('T4-18: cancel requested 优先 → process_exit', () => {
    const f = classifyFailure({
      exitCode: 1, killed: false, cancelRequested: true,
      stderrTail: 'auth failed', phase: 'running',
    });
    expect(f.code).toBe('process_exit');
  ...
  it('exit 0 但非 succeeded → process_exit', () => {
    const f = classifyFailure({
      exitCode: 0, killed: false, cancelRequested: false,
      stderrTail: '', phase: 'running',
    });
    expect(f.code).toBe('process_exit');
  ...
  it('stderr 含 rate limit → auth_failure（限流归入认证类）', () => {
    const f = classifyFailure({
      exitCode: 1, killed: false, cancelRequested: false,
      stderrTail: 'Error: rate limit exceeded, 429', phase: 'running',
    });
    expect(f.code).toBe('auth_failure');
  });
  ...
  it('stderr 含 upstream 错误 → 通用 process_exit', () => {
    const f = classifyFailure({
      exitCode: 1, killed: false, cancelRequested: false,
      stderrTail: 'service unavailable 503', phase: 'running',
    });
