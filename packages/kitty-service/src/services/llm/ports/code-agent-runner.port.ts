/**
 * Code Agent Runner Port
 *
 * 智能供应端的低级抽象：启动 agent 子进程并获得事件流。
 * 与 LlmProviderPort.generateReply()（HTTP API 模型回复）并列。
 */

import type { CodeAgentSession } from '../domain/code-agent-session';
import type { CodeAgentTaskContract } from '@kitty/contracts/code-agent/code-agent-task.contract';

export interface CodeAgentRunnerPort {
  /**
   * 提交任务。
   *
   ...
   */
  submit(task: CodeAgentTaskContract): Promise<CodeAgentSession>;
