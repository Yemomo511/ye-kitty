 ...
 */
async function probeAgent(def: CodeAgentDef): Promise<CodeAgentCapability> {
...
/** 尝试获取版本号，失败返回 null */
async function tryGetVersion(def: CodeAgentDef): Promise<string | null> {
  try {
...
function buildUnavailableReason(def: CodeAgentDef, detail: string): string {
  const parts = [detail];
  parts.push(`名称为 ${def.bin}（可通过 npm i -g ${def.id === 'claude-code' ? '@anthropic-ai/claude-code' : '@openai/codex'} 安装）`);
...
export async function refreshCapabilities(): Promise<readonly CodeAgentCapability[]> {
  const results = await Promise.allSettled(
    AGENT_DEFS.map((def) => probeAgent(def).catch((err): CodeAgentCapability => ({
      id: def.id,
      name: def.name,
      available: false,
...
/** 按 id 查找 AgentDef */
export function getAgentDef(id: string): CodeAgentDef | undefined {
