...
import type { CodeAgentFailure, FailureCode } from '../domain/code-agent-failure';
import { FAILURE_RETRYABLE } from '../domain/code-agent-failure';
...
const AUTH_FAILURE_RE = /(?:invalid.*(?:api[ -]?key|token|credential|auth)|not\s+logged\s+in|unauthorized|401|403|forbidden\b|auth.*fail|login.*fail|credential.*invalid|apikey.*invalid|api_key.*invalid|insufficient[_\s]permission|access[_\s]denied)/i;
const RATE_FAILURE_RE = /(?:rate[_\s]limit|too\s+many\s+requests|429|quota\s+exceed|billing|insufficient[_\s]quota|billing\s+not\s+active|usage\s+limit)/i;
const UPSTREAM_FAILURE_RE = /(?:service[_\s]unavailable|503|502|timeout|temporary[_\s]failure|overloaded|server[_\s]error|internal[_\s]server|upstream)/i;

// ---- 子进程退出码通用噪声跳过行 ----

const NOISE_LINES_RE = /(?:^\s*$|^\s*at\s+|debug:|info\b|trace:|sqlite|migration|listening|deprecation|notice:|warn(?:ing)?\b)/i;
