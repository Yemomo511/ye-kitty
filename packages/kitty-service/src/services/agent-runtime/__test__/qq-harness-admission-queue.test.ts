...
import { InMemoryQqHarnessAdmissionQueue } from '../application/in-memory-qq-harness-admission-queue';

describe('InMemoryQqHarnessAdmissionQueue', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  test.each([
    {
      ...
      errorMessage: 'QQ Harness准入最大并发数必须是正整数',
    },
    {
      config: { maxConcurrency: 2, maxQueueSize: 0 },
      errorMessage: 'QQ Harness准入队列容量必须是正整数',
    },
  ])('拒绝无效准入配置：$errorMessage', ({ config, errorMessage }) => {
    expect(() => new InMemoryQqHarnessAdmissionQueue(config)).toThrow(errorMessage);
  });

  test('全局最多同时执行两条消息，第三条等待令牌', async () => {
    const queue = new InMemoryQqHarnessAdmissionQueue({
      maxConcurrency: 2,
      maxQueueSize: 10,
    });
  ...
  test('任务异常后释放全局令牌和会话锁', async () => {
    ...
    const failureAssertion = expect(failedTask).rejects.toThrow('Harness执行失败');
    failureGate.resolve();
...
function enqueueBlockedTask(
  queue: InMemoryQqHarnessAdmissionQueue,
...
function createChatEvent(options: {
  ...
  readonly conversationId?: ChatEventContract['conversationId'];
}): ChatEventContract {
