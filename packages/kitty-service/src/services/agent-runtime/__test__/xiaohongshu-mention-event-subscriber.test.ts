...
import { XiaohongshuMentionEventSubscriber } from '../application/xiaohongshu-mention-event-subscriber';
...
class TestMentionChannel extends PlatformMessageService<XiaohongshuMentionEventContract> {
  async emit(event: XiaohongshuMentionEventContract): Promise<void> {
    await this.publishMessage(event);
  }
}

function createEvent(): XiaohongshuMentionEventContract {
  return {
