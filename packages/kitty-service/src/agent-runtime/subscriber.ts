...
import type { GroupChatCadenceDecision, GroupChatCadencePort } from '../platforms/qq/cadence-type';
import type { QqReplyAgentPort } from './runtime';
import type { MessageAdmission } from './queue';
import type { SkillContent, SkillRuntime } from './skills';
import { AdmissionQueue } from './queue';
import { QqReplyActionExecutor } from '../platforms/qq/reply';
 ...
 * 经过节奏门控和 Agent 准入队列后调用回复 Agent，并通过 QQ 发送端口回写。
 * 准入任务覆盖 Skill 选择、Agent、发送和节奏更新，保证同会话回复顺序。
 */
export class QqReplyEventSubscriber {
  private readonly actionExecutor: QqReplyActionExecutor;
  private readonly admissionQueue: MessageAdmission;

  constructor(
    private readonly qqMessageService: PlatformMessageService<PlatformMessage>,
    ...
    admissionQueue?: MessageAdmission,
  ) {
    this.actionExecutor = new QqReplyActionExecutor(botClient);
    this.admissionQueue = admissionQueue ?? new AdmissionQueue();
  }
  ...
  async handleMessage(message: PlatformMessage): Promise<void> {
    ...
    const admissionResult = await this.admissionQueue.enqueue({
      event: message,
      mentionsAgent,
      execute: async () => {
  ...
  // 在准入锁内完成Skill选择、Agent循环、QQ动作和节奏状态更新。
  ...
  private async loadQqChatSkill(message: PlatformMessage): Promise<SkillContent | undefined> {
    ...
    try {
      ...
      console.warn(
        `⚠️ [AgentRuntime-QQReplySubscriber-loadQqChatSkill] QQ默认Skill加载失败，已降级为普通Agent回复 skill=${QQ_CHAT_SKILL_NAME} conversationType=${message.conversationType} messageId=${maskId(
          message.message.id,
...
function isQqReceivedMessage(message: PlatformMessage): boolean {
  return message.platform === 'qq' && message.eventType === 'message.received';
}

// 群聊只把明确@机器人QQ号的消息交给模型，避免叶猫猫主动打断普通闲聊。
function isMentioningAgent(message: PlatformMessage, selfQqId: string | undefined): boolean {
