...
import { QqReplyEventSubscriber } from '../subscriber';
import { FallbackQqReplyAgent } from '../lm/fallback';
import { InMemoryConversationHistory } from '../history';
import { SafeQqReplyAgent } from '../safe';
import { SkillRuntime } from '../skills';
import type { GroupChatCadencePort } from '../../platforms/qq/cadence-type';
...
import type { QqReplyAgentInput, QqReplyAgentPort } from '../runtime';
...
describe('QqReplyEventSubscriber', () => {
  ...
  test('未@群聊命中节奏门控时强制触发Agent并先写入消息池', async () => {
    ...
    const cadenceEvents: PlatformMessage[] = [];
  ...
  test('准入队列丢弃消息时保留历史但不调用Agent或发送动作', async () => {
    ...
    const admissionQueue: MessageAdmission = {
      async enqueue(task) {
...
class TestQqMessageService extends PlatformMessageService<PlatformMessage> {
  async publish(message: PlatformMessage): Promise<void> {
...
function createChatEvent(
  options: {
    ...
    readonly platform?: PlatformMessage['platform'] | 'feishu';
  ...
  return {
    ...
    platform: (options.platform ?? 'qq') as PlatformMessage['platform'],
