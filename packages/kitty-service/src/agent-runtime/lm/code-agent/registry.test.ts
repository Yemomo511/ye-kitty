...
describe('code-agent-registry', () => {
  it('T5-1: AGENT_DEFS 含 claude-code + getAgentDef 查找', async () => {
    const { AGENT_DEFS, getAgentDef } = await import('@kitty/agent-runtime/lm/code-agent/registry');
...
describe('code-agent-logger', () => {
  it('T5-2: LoggerPort 实现存在且符合接口（info/warn/error 三方法）', async () => {
    const { log } = await import('@kitty/agent-runtime/lm/code-agent/log');
    expect(typeof log.info).toBe('function');
    expect(typeof log.warn).toBe('function');
    expect(typeof log.error).toBe('function');
    // 不抛错即通过
    log.info('test', { sessionId: 'test-1' });
  });
});

describe('failure-classifier 专项', () => {
  it('T5-3: classifyProtocolMismatch / WorkspaceFailure / PipeBroken', async () => {
    const { classifyProtocolMismatch, classifyWorkspaceFailure, classifyPipeBroken } =
      await import('@kitty/agent-runtime/lm/code-agent/classify');
