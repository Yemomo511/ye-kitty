...
/** 判断是否为终端事件。 */
export function isSessionEndEvent(event: CodeAgentEvent): event is CodeAgentSessionEndEvent {
  return event.type === 'session_end';
}

/** 判断是否为工具调用事件。 */
export function isToolUseEvent(event: CodeAgentEvent): event is CodeAgentToolUseEvent {
  return event.type === 'tool_use';
}

/** 判断是否为错误事件。 */
export function isErrorEvent(event: CodeAgentEvent): event is CodeAgentErrorEvent {
