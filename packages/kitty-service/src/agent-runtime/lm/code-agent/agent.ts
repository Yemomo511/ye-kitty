 ...
 * Code Agent运行核心
 *
 * 核心编排逻辑：实现 CodeAgentRunner，负责子进程生命周期、排队、
...
import type { CodeAgentSession, CodeAgentSessionStatus } from './session';
import type { CodeAgentFailure } from './failure';
import { CodeAgentSessionClosedError, CodeAgentPipeBrokenError } from './error';
import { getAgentDef } from './registry';
import { spawnAgent, cancelChild } from './process/session';
...
const GLOBAL_MAX_CONCURRENT = parseInt(
  process.env['CODE_AGENT_MAX_CONCURRENT_SESSIONS'] ?? '2',
  10,
);
...
interface SessionConsumerState {
  ...
  resolve: ((value: IteratorResult<CodeAgentEvent>) => void) | null;
}

// ---- Code Agent实现 ------------------------------------------

export class CodeAgent implements CodeAgentRunner {
  private readonly sessions = new Map<string, InternalSession>();
  private readonly eventCache = new Map<string, CodeAgentEvent[]>();
  ...
  async submit(task: CodeAgentTask): Promise<CodeAgentSession> {
    // 门禁
    ...
    log.info(`会话创建: ${id} agent=${def.id} status=queued`, { sessionId: id });
    ...
    if (this.canRunConcurrently(def)) {
      ...
      this.queue.push(id);
      log.info(`会话入队: ${id} queueDepth=${this.queue.length}`, { sessionId: id });
    }
  ...
  private canRunConcurrently(def: CodeAgentDefinition): boolean {
  ...
  private processQueue(): void {
    ...
    for (const id of this.queue) {
      ...
      if (session.queuedAt && now - session.queuedAt > QUEUE_TIMEOUT_MS) {
        ...
        log.warn(`会话排队超时: ${id}`, { sessionId: id });
        continue;
      }
  ...
  private async startSession(session: InternalSession, task: CodeAgentTask): Promise<void> {
    session.status = 'running';
    session.startedAt = Date.now();

    log.info(`正在启动子进程: ${session.agentDef.bin}`, { sessionId: session.id });
    ...
    child.on('error', (err) => {
      ...
      log.error(`子进程启动失败(spawn error): ${msg}`, err, { sessionId: session.id });
      session.failure = {
        code: 'spawn_failure',
        message: `子进程启动失败: ${msg}`,
        retryable: false,
    ...
    const { createClaudeMapper } = await import('./claude');
    const mapper = createClaudeMapper(session.id, (event: CodeAgentEvent) => {
  ...
  private startWatchdog(session: InternalSession): void {
    ...
    this.emitEvent = (s: InternalSession, event: CodeAgentEvent) => {
      if (s === session) {
  ...
  async cancel(sessionId: string): Promise<void> {
    ...
    if (session.status === 'queued') {
      ...
      log.info(`会话取消(queued): ${sessionId}`, { sessionId });
      return;
    }
    ...
    if (session.status === 'running' && session.child && !session.child.killed) {
      ...
      log.info(`会话取消(running): ${sessionId}`, { sessionId });
    }
  }
  ...
  private handleClose(
    ...
    log.info(`会话结束: ${session.id} status=${session.status} exitCode=${exitCode}`, {
      sessionId: session.id,
    });

    // 唤醒所有等待的消费者
    for (const consumer of session.consumers) {
      consumer.resolve?.({ value: undefined as unknown as CodeAgentEvent, done: true });
    }
  }
  ...
  private emitEvent(session: InternalSession, event: CodeAgentEvent): void {
    // ring buffer 写入
  ...
  private writeJsonl(sessionId: string, event: CodeAgentEvent): void {
    try {
      if (!existsSync(JSONL_LOG_DIR)) {
        mkdirSync(JSONL_LOG_DIR, { recursive: true });
      }
      appendFileSync(
        join(JSONL_LOG_DIR, `${sessionId}.jsonl`),
  ...
  private async *createEventStream(internal: InternalSession): AsyncIterable<CodeAgentEvent> {
    let cursor = 0;
    // 回放已有事件
    ...
    try {
      while (!internal.streamClosed) {
        const event = await new Promise<CodeAgentEvent>((resolve) => {
          consumer.resolve = (r: IteratorResult<CodeAgentEvent>) => {
            if (r.done) {
              resolve(null as unknown as CodeAgentEvent);
            } else {
              resolve(r.value);
            }
          };
...
/** 获取 AgentDef */
function getAgent(id: string): CodeAgentDefinition | undefined {
