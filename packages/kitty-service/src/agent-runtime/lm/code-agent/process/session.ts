...
import { resolveSpawnCommand } from './command';
import { buildAgentEnv } from './environment';
import { assertArgvBudget } from './args';
...
export function cleanupOrphans(): void {
  ...
  try {
    ...
    const records: PidRecord[] = lines
      .map((line) => {
        try {
          return JSON.parse(line) as PidRecord;
        } catch {
          return null;
        }
      })
    ...
    for (const record of records) {
      ...
      if (
        info.commandLine &&
        !info.commandLine.toLowerCase().includes(record.exeBasename.toLowerCase())
      ) {
