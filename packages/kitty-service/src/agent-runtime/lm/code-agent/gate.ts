...
export interface GateHook {
  ...
  check(task: CodeAgentTask): string | null | Promise<string | null>;
}
 ...
 */
function createWorkdirHook(workspaceRoot: string): GateHook {
...
export class Gate {
  ...
  registerHook(hook: GateHook): void {
    this.hooks.push(hook);
  }

  /** 执行门禁链。任一 Hook 返回拒绝原因即抛 CodeAgentGateRejectedError */
  async check(task: CodeAgentTask): Promise<void> {
